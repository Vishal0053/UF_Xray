...
def main(url):
    ...
    if not url_live(url):
        results["error"] = f"URL {url} is not live."
        return results
    ...
    ssl_details, ssl_error = get_ssl_certificate(url)
    if ssl_details:
        ssl_details_str = {str(k): str(v) for k, v in ssl_details.items()}
        results["ssl_details"] = ssl_details_str
    else:
        results["ssl_details"] = {"SSL Certificate Error": ssl_error}

    attack_results = test_url_for_attacks(url)
    detected_attacks = {attack: result for attack, result in attack_results.items() if result}
    results["detected_attacks"] = detected_attacks

    if is_suspicious_url(url, model):
        suspicious_str = f"The URL '{url}' is suspicious based on its structure."
    else:
        suspicious_str = f"The URL '{url}' appears to be safe based on its structure."
    results["suspicious"] = suspicious_str

    results["true_domain"] = get_true_domain(url)
